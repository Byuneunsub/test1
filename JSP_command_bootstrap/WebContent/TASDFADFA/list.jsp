	...
	<main id="contents">
		...
		<fieldset class="boardHead">
			...
			<div class="boardSearchForm">
				<form id="searchFrm" name="searchFrm" class="form-search" method="get" action="<%=request.getContextPath()%>/market/board/list.do">
		...
		<p class="boardListTxt">총 
		<b class="totalNum">${pageMaker.totalCount }</b>건 (
		<b>${pageMaker.cri.page }
		</b>/${pageMaker.realEndPage }page)
		</p>
		...
		<div class="btnArea btnRt">
			...
			<button class="btnTypeM btnLineType1" id="write" onclick="OpenWindow('<%= request.getContextPath()%>/market/board/registForm.do','상세보기',800,700)">글작성</button>
		</div>
			...
			<c:forEach items="${marketBoardList}" var="board">
				...
				<tr>
								...
								<td class="pc_Y ta_Y mo_Y title">
									<a href="javascript:OpenWindow('<%= request.getContextPath()%>/market/board/detail.do?bno=${board.bno}&from=list','상세보기',800,700)"> ${board.title} </a>
								</td>
							<!-- 첨부파일-->	
								<td></td>
							<!-- 부서 -->	
